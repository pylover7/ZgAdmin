...
describe("api/settings", () => {
  ...
  it("updateLoginConfig calls POST /api/v1/settings/login", () => {
    settingsApi.updateLoginConfig({
      qq: { app_id: "1", app_key: "k", redirect_uri: "/cb", enabled: true }
    });
    expect(mockRequest).toHaveBeenCalledWith("post", "/api/v1/settings/login", {
  ...
  it("deleteIPRules calls POST /api/v1/settings/security/ip-rules/delete", () => {
    ...
    expect(mockRequest).toHaveBeenCalledWith(
      "post",
      "/api/v1/settings/security/ip-rules/delete",
      {
        data: ["1", "2"]
      }
    );
