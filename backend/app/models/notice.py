...
class NoticeType(SQLModel):
    type: int = Field(default=0, description="通知类型：0-系统通知, 1-业务通知, 2-公告", schema_extra={"examples": [0]})


# ─── 创建 ───
class NoticeCreate(SQLModel):
    title: str = Field(max_length=200, description="通知标题", schema_extra={"examples": ["系统维护通知"]})
    content: str = Field(
        default="", description="通知内容", schema_extra={"examples": ["系统将于今晚 22:00 进行维护升级"]}
    )
    type: int = Field(default=0, description="通知类型：0-系统通知, 1-业务通知, 2-公告", schema_extra={"examples": [0]})
    level: str = Field(
        default="info", description="通知级别：info/warning/important", schema_extra={"examples": ["info"]}
    )
    status: int = Field(default=0, description="状态：0-草稿, 1-已发布", schema_extra={"examples": [1]})
...
class NoticeUpdate(BaseModel):
    ...
    content: str | None = Field(
        default=None, description="通知内容", schema_extra={"examples": ["维护时间调整为 23:00"]}
    )
    type: int | None = Field(
        default=None, description="通知类型：0-系统通知, 1-业务通知, 2-公告", schema_extra={"examples": [0]}
    )
    level: str | None = Field(
        default=None, description="通知级别：info/warning/important", schema_extra={"examples": ["warning"]}
    )
...
class NoticeFilter(SQLModel):
    title: str | None = Field(default=None, description="通知标题（模糊搜索）", schema_extra={"examples": ["维护"]})
    type: int | None = Field(
        default=None, description="通知类型：0-系统通知, 1-业务通知, 2-公告", schema_extra={"examples": [0]}
    )
    level: str | None = Field(
