...
class MenuCreate(SQLModel):
    ...
    title: str = Field(
        max_length=50,
        description="菜单名称（i18n键名或直接文字）",
        schema_extra={"examples": ["menus.notice"]})
    name: str = Field(
        max_length=50,
        description="路由名称（必须唯一）",
        schema_extra={"examples": ["Notice"]})
    path: str = Field(
        max_length=100,
        description="菜单路径",
    ...
    component: str = Field(
        ...
        description="前端组件路径",
        schema_extra={"examples": ["system/notice/index"]})
    rank: int = Field(
        default=1,
    ...
    dynamicLevel: int = Field(
        default=1,
        description="显示在标签页的最大数量",
        schema_extra={"examples": [1]})
