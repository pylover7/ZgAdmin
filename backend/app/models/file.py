...
class FileUpdate(BaseModel):
    name: str | None = Field(
        ...
        description="文件名（重命名）",
        schema_extra={"examples": ["季度报告.pdf"]})
...
class File(BaseModel, TimestampMixin, table=True):
    ...
    path: str = Field(
        max_length=500,
        description="存储相对路径",
        schema_extra={"examples": ["uploads/2026/05/a1b2c3d4.pdf"]})
    size: int = Field(
        default=0,
        description="文件大小（字节）",
        schema_extra={"examples": [1048576]})
    mime_type: str = Field(
        default="", max_length=100,
        description="MIME类型（magic检测）",
        schema_extra={"examples": ["application/pdf"]})
    file_type: str = Field(
        default="other",
        max_length=20,
        description="文件分类：image/document/video/audio/other",
    ...
    extension: str = Field(
        ...
        description="文件扩展名（小写）",
        schema_extra={"examples": ["pdf"]})
    uploader_id: UUID | None = Field(
