...
class SecurityPolicyBase(SQLModel):
    min_password_length: int = Field(
        default=8,
        description="最小密码长度（6-32）",
        schema_extra={"examples": [8]})
    ...
    password_history_count: int = Field(
        default=3,
        description="历史密码不可重用次数（0=不检查，最大24）",
        schema_extra={"examples": [3]})
    max_login_attempts: int = Field(
        default=5,
        description="最大登录失败次数（3-20）",
        schema_extra={"examples": [5]})
    lockout_duration_minutes: int = Field(
        default=30,
        description="账户锁定时长（分钟，5-1440）",
...
class SecurityPolicyUpdate(SQLModel):
    ...
    captcha_enabled: bool | None = Field(
        default=None,
        description="启用登录验证码",
        schema_extra={"examples": [True]})
...
class IPRuleBase(SQLModel):
    ip_cidr: str = Field(
        max_length=50,
        description="IP地址或CIDR，如 192.168.1.0/24",
        schema_extra={"examples": ["192.168.1.0/24"]})
    rule_type: str = Field(
        max_length=10,
...
class IPRuleUpdate(SQLModel):
    ...
    ip_cidr: str | None = Field(
        ...
        schema_extra={"examples": ["10.0.0.0/8"]})
