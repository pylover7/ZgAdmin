...
class UserBase(BaseModel):
    nickname: str = Field(
        ...
        nullable=True,
        description="用户昵称",
        schema_extra={"examples": ["张三"]})
    username: str = Field(
        max_length=20, unique=True,
        description="用户名称（登录账号）",
        schema_extra={"examples": ["zhangsan"]})
    ...
    password: str = Field(
        ...
        description="密码",
        schema_extra={"examples": ["MyP@ssw0rd123"]})
    phone: str = Field(
        default=None,
        max_length=20,
        nullable=True,
...
class User(UserBase, TimestampMixin, table=True):
    ...
    locked_until: datetime | None = Field(
        ...
        nullable=True,
        description="锁定截止时间",
        schema_extra={"examples": ["2026-05-26T11:00:00"]})
    password_history: list[str] | None = Field(
        default=None,
        sa_column=Column(JSON, nullable=True),
        description="最近N次密码hash列表，防止重复使用")
...
class UpdatePassword(SQLModel):
    current_password: str = Field(
        ...
        description="当前密码",
        schema_extra={"examples": ["OldP@ssw0rd"]})
...
class UserFiter(BaseModel):
    username: str | None = Field(
        ...
        schema_extra={"examples": ["zhang"]})
    email: str | None = Field(
        default=None,
        description="邮箱（模糊搜索）",
        schema_extra={"examples": ["example.com"]})
...
class UpdatePreferences(SQLModel):
    notify_account: bool | None = Field(
        default=None,
        description="账户相关变更通知",
        schema_extra={"examples": [True]})
    notify_system: bool | None = Field(
        default=None,
        description="系统公告和运维通知",
        schema_extra={"examples": [True]})
    notify_task: bool | None = Field(
        default=None,
        description="任务完成和审批通知",
        schema_extra={"examples": [True]})
